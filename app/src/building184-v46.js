/* 184 / Chengzeyuan 105: source long east-west plan and officially documented three flat-roof floors.
   Heights and openings remain explicit display fits pending attributable elevations. */
(function(Y){'use strict';
const A=Y.Architecture30,previous=A.render,F=Y.Footprints,ID='way/916931897',H=9.6,N=3;
function render(b,f,add){
 ...
 add('184-long-body',F.walls(g,.30,H),'#d5d9cc',24,id);
 add('184-long-plinth',F.walls(g,.03,.30),'#afb5a9',10,id);
 add('184-long-flat-roof',F.surface(g,H),'#73776a',22,id);
 // Native aerial shows the long rectangular plan; the narrow edge is a non-measured rim fit.
 add('184-long-roof-rim-fit',F.walls(g,H,H+.22),'#abb1a2',24,id);
 b.id=id;b.anim=0;
 try{
 ...
 return{profile:'184-original-long-three-floor-flat',observedFloors:N,height:H,heightMeasured:false,sourceFootprintPreserved:true,unseenFacadesVerified:false,entranceVerified:false};
}
Y.Building184={id:ID,heightFit:H,officialFloors:N,roof:'flat'};A.render=render;
})(YY);
