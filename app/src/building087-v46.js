/* Chinese Department, Humanities Building 6 main hall. The specific west
 * gallery, two-column porch, short Chinese plaque, right-hand vertical plaque,
 * green ground railing and recessed glass vestibule follow the department's
 * present website photos. Historic construction photos remain separate.
 * Heights, bay spacing, hidden facades and painted motifs are approximate. */
(function(Y){'use strict';
const A=Y.Architecture30,previous=A.render,G=Y.Geo,ID='way/939378519';
const O=[174.369,-476.571],R=-Math.atan2(28.502,3.194),CO=Math.cos(R),SI=Math.sin(R),W=Math.hypot(28.502,3.194),D=20.31320745993951;
const world=(x,z)=>[O[0]+x*CO+z*SI,O[1]-x*SI+z*CO],local=p=>[(p[0]-O[0])*CO-(p[1]-O[1])*SI,(p[0]-O[0])*SI+(p[1]-O[1])*CO];
const C={brick:'#929993',stone:'#c8c9be',red:'#ad4938',wood:'#854436',roof:'#656f68',tile:'#969f91',green:'#386b5b',blue:'#35586f',gold:'#c2ad79',glass:'#647977',dark:'#263c37'};
const H={base:.72,floor:4.55,eave:8.65,ridge:13.65},START=.60,END=W-.60,DEPTH=2.35,ENTRY=(START+END)/2;
function render(b,f){
 b.id=f.properties.pickId;const poly=f.geometry.coordinates[0].slice(0,-1).map(local);
 const group=(name,fn)=>{const old=b.e.add;b.e.add=function(k,...args){return old.call(this,'087-'+name+'-'+k,...args);};try{fn();}finally{b.e.add=old;}};
 const mesh=(name,g,col,mat=24)=>b.mesh('087-'+name,g,0,0,0,1,1,1,col,mat);
 ...
 function window(q){const y=(q.lo+q.hi)/2,h=q.hi-q.lo;
  ...
  for(let i=1;i<8;i++)b.box(q.x-q.w/2+i*q.w/8,q.hi-.23,.04,.027,.35,.087,C.wood,6);
 ...
 function names(){group('porch-names',()=>{
  // The department's present website photographs show the short horizontal
  // plaque and a single full-name vertical plaque on the viewer's right.
  // Reserve unused atlas rows without shifting subsequent shared sign slots.
  const px=3072,py=3840,paint=()=>{const q=b.ctx;q.save();q.clearRect(px,py,512,256);q.fillStyle='#49372b';q.fillRect(px,py,512,128);q.fillStyle='#d7ae5b';q.textAlign='center';q.textBaseline='middle';q.font='600 104px "Songti SC",serif';q.fillText('中文',px+256,py+65);q.fillStyle='#624531';q.fillRect(px+222,py+128,68,128);q.fillStyle='#d7ae5b';q.font='600 16px "Songti SC",serif';[...'中國語言文學系'].forEach((c,i)=>q.fillText(c,px+256,py+138+i*17));q.restore();};
  if(b.e.setAtlas){const old=b.e.setAtlas;b.e.setAtlas=function(canvas){this.setAtlas=old;const result=old.call(this,canvas),pixels=canvas.getContext('2d').getImageData(px/2,py/2,256,128).data;for(let i=3;i<pixels.length;i+=4)if(pixels[i])throw Error('087 label slot occupied');paint();if(this.gl)this.setAtlas(canvas);return result;};}else paint();
  ...
  label(ENTRY,4.55,D+3.56,3.35,.83,px,py,512,128);
  label(ENTRY+2.32,2.58,D+.22,.450625,2.06,px+242,py+128,28,128);
 });}
 ...
 function porch(){
  ...
  group('porch-column',()=>{for(const x of[ENTRY-half,ENTRY+half]){b.cyl(x,H.base,front,.285,4.24,C.red,12,1,6);b.cyl(x,H.base-.02,front,.32,.12,C.stone,12,1,24);}});
  group('porch-door',()=>{
 ...
 b.local(O[0],0,O[1],R,()=>{
  ...
  face('west-room',START,D-DEPTH,0,width,holes);face('west-north-return',0,D,0,START,[]);face('west-south-return',END,D,0,W-END,[]);
 ...
 return{id:ID,strategy:'building087-v46',floors:2,eaveHeight:H.eave,ridgeHeight:H.ridge,galleryDepth:DEPTH,entrance:world(ENTRY,D+3.15),originalOutline:true,limits:'East and end elevations, exact height and bay dimensions, roof ornaments and current on-site condition remain unverified.'};
}
A.render=function(b,f,add){return f.properties.id===ID?render(b,f):previous(b,f,add);};
Y.Building087={id:ID,render,world,local,width:W,depth:D,entry:ENTRY};
})(YY);
