/* 47-building only: six own plans, 16 asymmetrically arranged south bays, two upper balcony stacks, a southwest low hall and a ground electrical-room door. */
(function(Y){'use strict';
const F=Y.Footprints,A=Y.Architecture30,previous=A.render,ID='way/986745070';
const O=[-382.84,541.049],R=Math.atan2(2.965,53.497),CO=Math.cos(R),SI=Math.sin(R),H={floor:3.3,wall:19.8,hall:3.3};
const C={wall:'#bdc0b7',frame:'#dce0d5',glass:'#788b88',roof:'#999e93',stone:'#acb2a6',door:'#526762',service:'#677065'};
const world=(u,v)=>[O[0]+u*CO+v*SI,O[1]-u*SI+v*CO],local=p=>[(p[0]-O[0])*CO-(p[1]-O[1])*SI,(p[0]-O[0])*SI+(p[1]-O[1])*CO];
// Coordinates read from the 47-building plans in a 1880-pixel-wide view, fitted to its own eight-corner OSM footprint.
...
function render(b,f,add){const id=f.properties.pickId;b.id=id;
 ...
 const south=cells([428,511,591,672,753,836,916,997,1083,1166,1246,1327,1412,1491,1572,1652,1736].map(planX));
 const north=cells([428,511,591,672,744,846,916,997,1083,1166,1246,1318,1421,1491,1572,1652,1736].map(planX));
 const west=cells([392,475,555,635,715,799].map(wingY));
 const inner=cells([392,475,555,635,681].map(y=>(y-392)/(681-392)*11.398734));
  ...
  b.local(a[0],0,a[1],Math.atan2(-dz,dx),()=>group('facade-'+name,()=>{
   ...
   for(let floor=0;floor<floors;floor++){const base=floor*H.floor,qs=openingsForFloor(floor,len).sort((a,b)=>a.s-b.s);let cursor=0;
    for(const q of qs){const bottom=base+(q.door?(floor===0?.3:.12):.85),top=base+2.7;
     ...
     b.box(x,y,-depth,w-.08,h-.06,.055,q.service?C.service:q.door?C.door:C.glass,q.service?24:5);
 ...
 for(let level=0;level<6;level++)add('099-floor-'+level,F.surface(f.geometry,level*H.floor+.12),C.stone,24,id);
 add('099-source-roof',F.surface(f.geometry,H.wall),C.roof,24,id);
 for(let i=1;i<ring.length;i++){let a=ring[i-1],c=ring[i];if(positive)[a,c]=[c,a];const la=local(a),lc=local(c),du=lc[0]-la[0],dv=lc[1]-la[1],horizontal=Math.abs(du)>Math.abs(dv),mx=(la[0]+lc[0])/2,mv=(la[1]+lc[1])/2,len=Math.hypot(c[0]-a[0],c[1]-a[1]);let name='return',source=[];
  ...
  else if(!horizontal){name=mx<1?'wing-west':mx<20?(mv>17?'bar-west':'wing-east'):'east';source=name==='wing-west'?west:name==='wing-east'?inner:[[17.32,19.05]];}
  ...
  facade(a,c,name,(floor)=>{let qs=positions.map(q=>({...q}));
   if(name==='wing-west')qs=qs.flatMap(q=>q.original>wingY(475)?[{...q,e:q.s+.78,door:true},{...q,s:q.s+.88}]:[q]);
   if(floor===0&&name==='bar-west')qs=[];
   if(name==='south')for(const door of entrances.slice(0,2)){const x=(door.u-la[0])/du*len,w=floor===0?door.width:1.35,s=x-w/2,e=x+w/2;qs=qs.filter(q=>q.e<s-.1||q.s>e+.1);qs.push({s,e,door:true,service:floor===0&&door.service});}
   return qs;
  });
 }
 b.local(O[0],0,O[1],R,()=>{
  // West-wing outside strip is present on all six own plans, except the north end room.
  group('wing-balconies',()=>{const s=wingY(475),e=17.047877,w=e-s;
  ...
  group('southwest-hall',()=>{const u0=.013613,u1=12.612314,v0=17.047877,v1=24.774914,door=entrances[2],s=door.u-door.width/2,e=door.u+door.width/2;
