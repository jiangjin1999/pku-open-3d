/* North Pavilion: own two-storey square-point roof; west door placement remains an explicit hypothesis. */
(function(Y){'use strict';const F=Y.Footprints,G=Y.Geo,A=Y.Architecture30,previous=A.render,ID='way/272353673';
const O=[-228.242,15.523],R=Math.atan2(.367,18.243),CO=Math.cos(R),SI=Math.sin(R),CX=9.124,CZ=9.267;
const H={plinth:.55,lowerWall:4.25,lowerEave:4.65,lowerInner:6.05,upperWall:8.45,upperEave:8.75,apex:12.15,finial:13.50};
...
for(const upper of[false,true]){const label=upper?'upper-pointed':'lower-annular',mesh=new G.Geometry(),tiles=new G.Geometry();for(let side=0;side<4;side++){for(let i=0;i<32;i++)for(let j=0;j<12;j++){const t0=-1+i/16,t1=t0+1/16,r0=j/12,r1=(j+1)/12,p=[roofPoint(side,t0,r0,upper),roofPoint(side,t1,r0,upper),roofPoint(side,t1,r1,upper),roofPoint(side,t0,r1,upper)];mesh.quad(...p.map(p=>vertex(...p)));roofTriangles[upper?'upper':'lower'].push([p[0],p[1],p[2]],[p[0],p[2],p[3]]);}// Tile courses are parallel within each triangular slope, ending at the hip instead of radiating from the apex.
 const outer=side%2?(upper?8.82:9.267):(upper?8.65:9.124),inner=upper?0:(side%2?6.2:6.0),n=Math.ceil(outer*2/.30),half=.085;
 ...
 }add('048-'+label+'-roof',mesh,C.roof,25,id);add('048-'+label+'-tile-ribs',tiles,C.tile,25,id);}
b.local(O[0],0,O[1],R,()=>b.local(CX,0,CZ,0,()=>{
 ...
 // Photo facade is provisionally placed west. A separate hollow planter is not a stair flight.
 group('west-door-provisional',()=>{b.box(-6.85,.275,0,.55,.55,2.3,C.stone,24);});
 group('photo-stone-planter',()=>{const x=-7.65,w=1.15,d=3.8;b.box(x,.12,0,w,.12,d,C.stone,24);for(const z of[-d/2,d/2])b.box(x,.46,z,w,.56,.16,C.stone,24);for(const xx of[x-w/2,x+w/2])b.box(xx,.46,0,.16,.56,d,C.stone,24);b.box(x,.21,0,w-.22,.06,d-.22,'#5f654d',24);});
}));
return{strategy:'building048-v46',floors:2,lowerRoofAnnular:true,roofApexCount:1,longRidge:false,doorDirectionVerified:false,provisionalDoorFace:'west',planterNotStair:true,heightMeasured:false,outlineMeansEstimatedEaveProjection:true};
}
A.render=function(b,f,add){return f.properties.id===ID?render(b,f,add):previous(b,f,add);};Y.Building048={id:ID,render,world,local,heights:H,center:[CX,CZ]};
})(YY);
